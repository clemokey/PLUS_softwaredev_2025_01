...
This script allows the user to provide an address or place name,
and it will geocode the address to find its geographic coordinates.
It then retrieves the user's current approximate geographic location
and calculates the route from the user's location to the specified address
using the OpenRouteService API. The script can return either an interactive
map showing the route or a text summary of the route directions, distance,
duration, and estimated time of arrival (ETA).

This script currently accepts a single address, it doesn't accept coordinates of the destination.

To get the best results, use conda to create the environment using the environment.yml file provided.

This file can also be imported as a module and contains the following
functions:

    * gestart_location - retrieves the user's current approximate geographic location
    * geocode_address - geocodes a given address string into geographic coordinates
    * get_route - requests route directions from OpenRouteService API
    * create_route_map - creates an interactive map displaying a route with start and end markers
    * get_route_directions_str - generates step-by-step directions and summary information from a route
"""
